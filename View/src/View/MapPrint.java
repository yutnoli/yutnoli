...
//이미지를 버튼의 크기에 맞게 조절하는 부분
		...
		icon = new ImageIcon("./yut_backdo.png");
		img = icon.getImage();
		resizedImage = img.getScaledInstance(20, 70, Image.SCALE_SMOOTH);
		yutIcon[2] = new ImageIcon(resizedImage);
		//throw Image
...
//이미지 넣은 버튼 생성
		...
		for(int i = 0; i < 2; i++) {
			...
			normalState[i+22].setBounds(555 + i*85, 555 + i*85, 50, 50);
		}
...
//panel에 button add		
	...
	public void yutImageChange() {
		...
		if(yut[3] == 0) {
			yutPosition[3].setIcon(yutIcon[0]);
		}
		else {
			yutPosition[3].setIcon(yutIcon[2]);
		}
	}
...
class GetPosition extends JFrame{
	JButton[] getSelectButton = new JButton[6];
	...
	void initPositionFrame(ControlMain controler, MapPrint mapPrint) {
		...
		getSelectButton[5] = new JButton("Back-Do");
