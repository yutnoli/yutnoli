...
class SelectMove implements ActionListener{
	...
	GameEndFrame gameEnd = new GameEndFrame();
	
	
	void initSelcetMove(JButton[] getPositionButton, JFrame selectFrame, ControlMain controler, MapPrint mainMap) {
		this.getPositionButton =  getPositionButton;
		this.selectFrame = selectFrame;
		this.controler = controler;
		this.mainMap = mainMap;
		gameEnd.initPositionFrame(mainMap);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(delete.deleteSelectedMove(selected)) {
			...
			if(data.getEndCheck() != 9) {
				...
				gameEnd.showPositionFrame(data.getEndCheck());
			}
		}
		else {
...
public class GetEndMove extends JFrame{
	...
	void initPositionFrame(ControlMain controler, MapPrint mainMap) {
		...
		position.initSelcetMove(getSelectButton, this, controler, mainMap);
